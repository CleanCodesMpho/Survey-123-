...
# TEMPLATE
# =========================
TEMPLATE_PATH = "CHEMICAL SAFETY Report.docx"  # change if your file has a different name
...
def convert_docx_to_pdf(docx_file):
    ...
    if not os.path.exists(pdf_file):
        raise Exception("PDF conversion failed: output PDF not found")
...
def upload_report_to_agol(file_path, objectid):
    ...
    item_properties = {
        ...
        "snippet": f"Automatically generated PDF report for Survey123 submission {objectid}"
    }
...
def generate_report(attributes, objectid):
    ...
    # Uses your actual field names directly
    context = {k: ("N/A" if v in [None, ""] else v) for k, v in attributes.items()}
    context["inspection_date"] = edit_date
    ...
    # Upload PDF to AGOL
    real_url = upload_report_to_agol(pdf_file, objectid)

    # Cleanup temp files
    for path in [docx_file, pdf_file, qr_file]:
        try:
...
async def survey_webhook(request: Request):
    ...
    try:
        ...
        # STEP 5: generate PDF report and upload to AGOL
